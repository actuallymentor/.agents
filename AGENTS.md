...
- You may only push to repositories (with either `git` or `gh`) when explicitly asked to do so, for auth check environment variables, then `.env.local`, then `.ssh_key` for a key
