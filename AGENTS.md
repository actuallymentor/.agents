...
- You may only push to repositories (with either `git` or `gh`) when explicitly asked to do so, for auth check `.env.local`, then `.ssh_key`, then environment variables for a key
